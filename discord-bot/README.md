...
**Fun:** `/meme` `/meme-ro` (random pick across several Romanian meme subreddits: RomaniaDank, Romania, CredemInSuveranitate, RomaniaBun, Bucuresti — with general meme subreddits as a backup if those ever fail) `/8ball` `/coinflip` `/guess` — plus an automatic Romanian meme every 20 minutes in the memes channel (adjustable via `MEME_INTERVAL_MINUTES`), auto-reacted with 👍/👎 so people can vote. Manually posted memes get the same vote reactions.
